...
- `directional-ensemble (1d)` — FAILED — significantly worse than the naive baseline
  - **verdict not supported by its own interval** — accuracy 0.4412 [0.3444, 0.5426] and its null 0.5552 [0.3863, 0.7124] OVERLAP across [0.3863, 0.5426]: the verdict compares the accuracy interval to the null's point estimate and ignores the null's own published interval, so the stated skill of -0.1140 is not resolved by this sample
...
- `directional-ensemble (1w, high conviction)` — INSUFFICIENT DAYS (3/10 credible days of 17, 14 degenerate) — no interval, so no verdict
...
- `liquidity21-crypto` — registered claim 79.5%, 184 forecasts recorded, 0 graded. Not a live result.
- `trend21` — registered claim 73.1%, 8,005 forecasts recorded, 0 graded. Not a live result.
- `trend21-crypto` — registered claim 93.4%, 184 forecasts recorded, 0 graded. Not a live result.
...
**Multiplicity:** family_size=13, looks=39, divisor=507, corrected_alpha=9.861932938856017e-05.
