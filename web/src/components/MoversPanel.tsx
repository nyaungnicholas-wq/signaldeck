...
import { movers, type MoverRow, type MoversResponse } from "@/lib/api";
...
function MoverTable({
  ...
  return (
    <div className="min-w-0">
      <div
        className="px-3 py-1.5 text-[0.68rem] font-bold tracking-[0.14em]"
        style={{ color }}
      >
        {title}
      </div>
      {rows.length === 0 ? (
        <p className="px-3 pb-2 text-[0.72rem]" style={{ color: "var(--faint)" }}>
          none in this filter
        </p>
      ) : (
              ...
              <tr style={{ borderBottom: "1px solid var(--border)" }}>
                {["SYM", "LAST", "CHG%", "MCAP"].map((h, i) => (
                  <th
                    key={h}
                    scope="col"
                    className={`px-3 py-1 text-[0.65rem] font-medium tracking-wide ${i === 0 ? "text-left" : "text-right"}`}
  ...
  return (
    <section className="panel" aria-label="top gainers and losers">
      ...
      <div className="flex flex-wrap items-center gap-1.5 px-3 py-2">
        <span className="text-[0.65rem] tracking-wider" style={{ color: "var(--faint)" }}>
          MCAP
        </span>
        {MCAP_FILTERS.map((f) => (
          <button
            ...
            className="chip min-h-[36px] cursor-pointer px-2 py-[2px] text-[0.68rem] transition-colors duration-150 hover:brightness-125"
            style={
              minMcap === f.min
