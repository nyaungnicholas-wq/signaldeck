    ...
    <div className="flex flex-col gap-4">
      <section className="panel">
        ...
        {stored.length === 0 ? (
          <EmptyState
            className="m-4"
            message="No 13F books stored yet"
            detail="The 13f-poller rotates through the curated managers daily (a few per run, rate-limited per SEC policy) — books appear as it sweeps."
          />
        ) : (
          <ul style={{ borderTop: "1px solid var(--border)" }}>
            {stored.map((m) => (
              <li key={m.cik} style={{ borderBottom: "1px solid var(--border)" }}>
                <Link
                  href={`/institutions?manager=${encodeURIComponent(m.cik)}`}
        ...
        <section className="panel">
          ...
          <div className="flex flex-wrap gap-2 px-4 py-3">
            {pending.map((c) => (
              <Link
                key={c.cik}
                href={`/institutions?manager=${encodeURIComponent(String(c.cik))}`}
...
function InstitutionsInner() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {manager ? <ManagerHoldings manager={manager} /> : <ManagersOverview />}
    </div>
  );
}
