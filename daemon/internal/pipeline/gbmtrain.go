...
func modelLegRankEdge(vetoed map[string]bool, edges map[string]float64, models []store.ModelForecast, h md.Horizon, name string, now int64) (float64, bool) {
	for _, m := range models {
		if m.Horizon == h && m.Model == name {
			if now-m.Ts > maxModelForecastAgeSecs {
				return 0, false
			}
			return rankGate(vetoed, edges, name, h, m.AUC, m.NEval)
		}
	}
	return 0, false
}
