// Package universe holds the curated broad free-tier daily universe: a
// hardcoded list of liquid US large/mid-cap tickers (approximately the S&P 500
// constituents) plus the universe-poller worker that fetches DAILY bars for
// them via Alpaca's free multi-symbol bars endpoint. These names are stored as
// active-for-daily but NOT streamed, so the cross-sectional
// ranking/correlation/regime models and the per-symbol daily models get wide
// coverage at ZERO marginal cost (daily bars are tiny; the free IEX feed and
// its 200 req/min ceiling more than suffice — see the budget note on the
// worker).
package universe

// SP500 is a curated slice of liquid US large/mid-cap tickers, approximately
// the S&P 500 constituents (membership drifts; this is intentionally
// approximate — it only needs to be a broad, liquid, free cross-section, and
// any symbol Alpaca rejects is simply skipped by the poller). Deduplicated,
// upper-case, no dotted class shares that the IEX feed doesn't carry.
//
// This is the SEED for the SIGNALDECK_UNIVERSE_CAP-bounded daily universe; the
// cap trims it (highest-priority-first == list order) so operators can dial
// coverage down without editing code.
var SP500 = []string{
