...
- R24 VERIFIED (2026-09-07 12:40 local, after Nicholas ran fix-web-task.ps1 + restart-web.ps1 elevated): 8323 is held by node started via the launcher (`next start -H 127.0.0.1 -p 8323`), owner Nicholas_N, task principal Interactive/Limited, orphan PID 4676 gone; /login 200, /health 200, unauthenticated bars 401, keyed bars via 8323 200, /api/version = e38b39f; LAN address refused (loopback-only); an unelevated Stop/Start of the task replaced the listener (pid 39572 -> 13524) in 5s with the old process dead. Ports 3000 and 8323 are now identical loopback keyed instances; either may be retired.
...
## Second pass (2026-09-07 afternoon, "fix them all")
- R26 FIXED: Stock Watcher mirrors are dead, but Kadoa's Congress Trading Monitor publishes a free, keyless, daily JSON (house_clerk + senate_efd rows, newest ~5,000). `ingest/congress/kadoa.go` maps it onto the existing Trade shape (same TradeID recipe, OGE rows dropped); `pipeline/congress.go` falls back to it whenever a mirror fetch fails; `SIGNALDECK_CONGRESS_KADOA_URL` overrides or disables ("off"). Live opt-in test (`SIGNALDECK_LIVE_KADOA=1`): senate 938 trades (newest 2026-09-02), house 679 (newest 2026-09-03). Same fragility class as Stock Watcher (one volunteer repo), so the mirrors stay primary.
- R25 still BLOCKED, now with the exact reason: this machine has ONE physical disk (C:, 1.9 TB), no external or network volume, and OneDrive is installed but NOT signed in (no account in the registry, Availability status blank), so `OneDrive\SignalDeckBackups` (9.7 GB) never leaves the machine. Two ways to close it: sign in to OneDrive (then the folder syncs; the script still needs SIGNALDECK_OFFSITE_DIR on another volume or an S3 target to COUNT it) or set `SIGNALDECK_OFFSITE_S3`.
- R15 completed: loadBars now trims the forming daily bar for every caller (signal-runner, expectancy-runner, prediction-runner, CurrentState) and the five direct store reads (regime-runner, ranking-runner, HMM regime, expectancy trainer labels, cross-sectional features) trim too.
- R16 test added (`predict_evidence_test.go`: two withheld symbols -> Symbols=2, never served as a forecast).
- Paper D2/D3 durable: a position with no daily close and a stranded exit now write dq_events (`paper_unmarked_position`, `paper_stranded_exit`) instead of only a log line.
- Paper D5 FIXED: the API's round-trip reconstruction used one open lot per symbol; the manual book adds to positions and sells partially, so `api/paperroundtrips.go` now uses papertrade.MatchRoundTrips (FIFO), the same matcher the sizing edge uses.
- Accuracy gate: the newest-N reconstruction could FAIL OPEN when the grader's settle-day window trailed the call-day slice; it now checks N+10 sessions (a superset), so a collapsed day can only over-refuse. The exact-day-set version (repro/directional_days.csv) is documented in the investigation but not implemented: the CSV is settle-day keyed and can lag the registry by weeks.
