...
`9dd47de`, `d8cca71`. **The daemon repairs are committed but NOT deployed**: the sanctioned path
`bash ops/signaldeck-ctl.sh deploy` was blocked by the session's permission classifier and must be run by
Nicholas (it re-runs `go test ./...`, builds from the commit, restarts, and verifies `/api/version`; it refuses a
dirty tree, so commit or stash the research files first). Until then `worker_runs.revision` stays `d08d3f1` and
none of F1-F4 is live. Rollback: `git revert 87e7f57` then the same deploy command.

**Status: repairs complete and tested; research campaign in progress; publication status unchanged (REFUSED).**
...
- Deploy of 87e7f57 (permission classifier); the running daemon is `d08d3f1` until Nicholas runs
  `bash ops/signaldeck-ctl.sh deploy`.

## Not verified

- The rv runner's detail string on a live pass (needs the deploy); expected first live line:
  `froze 0 forecast(s) ... (850 already frozen by an earlier pass); N symbol(s) skipped: last bar forming or stale`.
