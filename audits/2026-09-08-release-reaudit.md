...
| **F13** | provenance | medium | Docs call the anchors repo public; GitHub says PRIVATE | gh repo view 2026-09-09: nyaungnicholas-wq/signaldeck-anchors visibility PRIVATE. The mismatch is closed from the docs side: README line 154 says the repository is private today, docs/PUBLIC_RELEASE_PLAN.md row "Make signaldeck-anchors repo public" records the 2026-09-09 check, and ops/anchor-publish.sh no longer calls it the public anchors repo (2 wording lines). Making it public stays an owner option in the release plan, not a defect | fixed |
...
| **F19** | ops | medium | The SignalDeck Web task (8323) dies with 0xC000013A when a console control event reaches it; the principal fix needs elevation. Re-checked 2026-09-09 03:50 unelevated: Set-ScheduledTask -Principal S4U on the task and Register-ScheduledTask of a fresh S4U task both return "Access is denied", so no owner-level route exists. Reproduction attempt 2026-09-09 02:43-02:49 PT: the full Playwright suite (43 passed, 2 skipped, own server on 8329, the recorded trigger) ran to completion while the 8323 node process kept the same pid (18292) and answered 200 throughout; web-guard.ps1 has logged 0 restarts in 189 probes since 2026-09-08 evening. Residual: a console-control kill would cost at most one 5-minute keepalive interval; the S4U hardening stays available via an elevated ops/fix-task-principals.ps1 run | FIXED 2026-09-09 16:47-16:51 PT: Nicholas ran ops/fix-task-principals.ps1 elevated. Get-ScheduledTask read-back: all 19 SignalDeck tasks LogonType S4U (Web, Local Workspace, Web Keepalive and Daemon were Interactive); Microsoft-Windows-TaskScheduler/Operational enabled: true; SignalDeck Web Running since 16:51:21 with 8323 and 3000 answering 200, /api/ready 200, health.json ok. Earlier evidence: At 2026-09-08 00:50 port 8323 refused connections; the task "SignalDeck Web" showed last result 0xC000013A (console control exit) at 23:53 while the loopback workspace on 3000 stayed up. ops/check-task-health.ps1 has ... | fixed |
