...
Verified this run: 57.1% of graded outcomes carry a frozen baseline.
...
the daemon stops publishing it. Chained digest matches the graded rule: 353450b995a52bcea9257a8520369d8841e7fa71c70295aae925dc7cf03b429f.
  directional-ensemble (1d): n=2,768 over 24 days, design effect 7.2x -> effective n 384
  prequential-majority (1d): n=2,439 over 22 days, design effect 22.3x -> effective n 110
...
Reliability (predicted P(up) vs realized up-frequency, independent symbol-days):
  ...
  1d: p in [0.4,0.5) mean 0.469 -> realized 0.571 (n=1635, 22 days)
  ...
  1w: p in [0.0,0.1) mean 0.034 -> realized 0.308 (n=13, 6 days) <- conviction tier
  1w: p in [0.1,0.2) mean 0.171 -> realized 0.673 (n=98, 12 days) <- conviction tier
  1w: p in [0.2,0.3) mean 0.259 -> realized 0.543 (n=127, 17 days) <- conviction tier
  1w: p in [0.3,0.4) mean 0.363 -> realized 0.609 (n=1193, 23 days)
  1w: p in [0.4,0.5) mean 0.448 -> realized 0.581 (n=2194, 18 days)
  1w: p in [0.5,0.6) mean 0.544 -> realized 0.562 (n=1177, 17 days)
  1w: p in [0.6,0.7) mean 0.631 -> realized 0.606 (n=193, 13 days)
  1w: p in [0.7,0.8) mean 0.742 -> realized 0.353 (n=51, 11 days) <- conviction tier
  1w: p in [0.8,0.9) mean 0.846 -> realized 0.375 (n=8, 4 days) <- conviction tier
...
Universe completeness (direction): listing status resolvable for all 1,043 graded symbols — survivorship_clean.
Universe completeness (structure): listing status resolvable for all 418 graded symbols — survivorship_clean.
Stale-feed quarantine: 1,737 graded observation(s) over 460 symbol(s) and 25 trading day(s) EXCLUDED - minted on a day this symbol's own feed was flagged stale (dq_events kind='stale'), so the inputs behind the call had already been declared unfit. Grading them would measure the outage, not the predictor.
Survivorship bound (measured 2026-08-29T02:23:02Z, edgar:form-25): 9 symbol(s) left the tracked universe since 2026-07-24 vs 1,043 graded. If every dropped symbol
had kept being graded and been WRONG every time, headline accuracy would fall by at most 0.43 pp.
