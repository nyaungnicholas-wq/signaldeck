...
the daemon stops publishing it. Chained digest matches the graded rule: 353450b995a52bcea9257a8520369d8841e7fa71c70295aae925dc7cf03b429f.
  directional-ensemble (1d): n=2,766 over 24 days, design effect 7.2x -> effective n 384
  prequential-majority (1d): n=2,432 over 21 days, design effect 22.4x -> effective n 109
...
Reliability (predicted P(up) vs realized up-frequency, independent symbol-days):
  ...
  1d: p in [0.4,0.5) mean 0.469 -> realized 0.570 (n=1634, 22 days)
  1d: p in [0.5,0.6) mean 0.552 -> realized 0.554 (n=386, 16 days)
  ...
  1w: p in [0.1,0.2) mean 0.175 -> realized 0.667 (n=87, 9 days) <- conviction tier
  1w: p in [0.2,0.3) mean 0.258 -> realized 0.560 (n=116, 14 days) <- conviction tier
  1w: p in [0.3,0.4) mean 0.363 -> realized 0.612 (n=1149, 20 days)
  1w: p in [0.4,0.5) mean 0.448 -> realized 0.584 (n=2124, 15 days)
  1w: p in [0.5,0.6) mean 0.545 -> realized 0.563 (n=1118, 14 days)
  1w: p in [0.6,0.7) mean 0.629 -> realized 0.606 (n=155, 10 days)
  1w: p in [0.7,0.8) mean 0.733 -> realized 0.333 (n=30, 8 days) <- conviction tier
  1w: p in [0.8,0.9) mean 0.841 -> realized 0.400 (n=5, 3 days) <- conviction tier
...
Universe completeness (direction): listing status resolvable for all 655 graded symbols — survivorship_clean.
Universe completeness (structure): listing status resolvable for all 285 graded symbols — survivorship_clean.
Stale-feed quarantine: 404 graded observation(s) over 69 symbol(s) and 22 trading day(s) EXCLUDED - minted on a day this symbol's own feed was flagged stale (dq_events kind='stale'), so the inputs behind the call had already been declared unfit. Grading them would measure the outage, not the predictor.
Survivorship bound (measured 2026-08-27T01:37:55Z, edgar:form-25): 8 symbol(s) left the tracked universe since 2026-07-24 vs 655 graded. If every dropped symbol
had kept being graded and been WRONG every time, headline accuracy would fall by at most 0.58 pp.
