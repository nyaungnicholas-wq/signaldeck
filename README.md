...
the daemon stops publishing it. Chained digest matches the graded rule: 353450b995a52bcea9257a8520369d8841e7fa71c70295aae925dc7cf03b429f.
  directional-ensemble (1d): n=2,649 over 21 days, design effect 7.4x -> effective n 360
  prequential-majority (1d): n=2,315 over 18 days, design effect 23.5x -> effective n 98
  directional-ensemble (1w): n=4,563 over 19 days, design effect 12.6x -> effective n 363
  prequential-majority (1w): n=4,202 over 17 days, design effect 13.3x -> effective n 316
  directional-ensemble (1w, high conviction): n=497 over 16 days, design effect 2.8x -> effective n 175
...
Reliability (predicted P(up) vs realized up-frequency, independent symbol-days):
  ...
  1d: p in [0.3,0.4) mean 0.372 -> realized 0.634 (n=344, 9 days)
  1d: p in [0.4,0.5) mean 0.468 -> realized 0.575 (n=1521, 19 days)
  1d: p in [0.5,0.6) mean 0.552 -> realized 0.552 (n=382, 15 days)
  ...
  1w: p in [0.0,0.1) mean 0.044 -> realized 0.375 (n=8, 3 days) <- conviction tier
  1w: p in [0.1,0.2) mean 0.176 -> realized 0.667 (n=81, 7 days) <- conviction tier
  1w: p in [0.2,0.3) mean 0.258 -> realized 0.575 (n=106, 11 days) <- conviction tier
  1w: p in [0.3,0.4) mean 0.364 -> realized 0.621 (n=1107, 18 days)
  1w: p in [0.4,0.5) mean 0.448 -> realized 0.587 (n=2067, 13 days)
  1w: p in [0.5,0.6) mean 0.545 -> realized 0.563 (n=1056, 11 days)
  1w: p in [0.6,0.7) mean 0.626 -> realized 0.664 (n=122, 7 days)
  1w: p in [0.7,0.8) mean 0.726 -> realized 0.438 (n=16, 5 days) <- conviction tier
...
Universe completeness (direction): listing status resolvable for all 640 graded symbols — survivorship_clean.
Universe completeness (structure): NOT CLEAN — unmeasured — no graded post-epoch symbols. Rows carry survivorship_clean=false.
Stale-feed quarantine: 388 graded observation(s) over 65 symbol(s) and 22 trading day(s) EXCLUDED - minted on a day this symbol's own feed was flagged stale (dq_events kind='stale'), so the inputs behind the call had already been declared unfit. Grading them would measure the outage, not the predictor.
Survivorship bound (measured 2026-08-22T01:32:54Z, edgar:form-25): 9 symbol(s) left the tracked universe since 2026-07-24 vs 640 graded. If every dropped symbol
had kept being graded and been WRONG every time, headline accuracy would fall by at most 0.64 pp.
