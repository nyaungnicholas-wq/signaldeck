...
Verified this run: 63.6% of graded outcomes carry a frozen baseline.
...
Reliability (predicted P(up) vs realized up-frequency, independent symbol-days):
  ...
  1w: p in [0.2,0.3) mean 0.259 -> realized 0.551 (n=127, 17 days) <- conviction tier
  1w: p in [0.3,0.4) mean 0.363 -> realized 0.607 (n=1189, 24 days)
  1w: p in [0.4,0.5) mean 0.448 -> realized 0.582 (n=2194, 18 days)
  1w: p in [0.5,0.6) mean 0.544 -> realized 0.559 (n=1180, 17 days)
  1w: p in [0.6,0.7) mean 0.630 -> realized 0.601 (n=198, 14 days)
  1w: p in [0.7,0.8) mean 0.740 -> realized 0.380 (n=50, 11 days) <- conviction tier
  1w: p in [0.8,0.9) mean 0.851 -> realized 0.273 (n=11, 7 days) <- conviction tier
...
Universe completeness (structure): listing status resolvable for all 419 graded symbols — survivorship_clean.
Stale-feed quarantine: 1,740 graded observation(s) over 460 symbol(s) and 26 trading day(s) EXCLUDED - minted on a day this symbol's own feed was flagged stale (dq_events kind='stale'), so the inputs behind the call had already been declared unfit. Grading them would measure the outage, not the predictor.
Survivorship bound (measured 2026-08-31T23:24:24Z, edgar:form-25): 9 symbol(s) left the tracked universe since 2026-07-24 vs 1,043 graded. If every dropped symbol
