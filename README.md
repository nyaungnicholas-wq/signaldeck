...
> The grader refused at 2026-07-29T14:05:00 (research-loop liveness check failed (exit 1) — a narrated grid search left no verifiable judgment record, or a pre-registered forecast kind has never frozen a forecast and gave no refusal; the grader was not run). The last successful grade was 2026-07-27T01:01:42 (61.1h old). The previously published tables have been REMOVED rather than reprinted, because a number graded by code that refused to run today is not a live number.

```
RESEARCH-LOOP LIVENESS FAILED — 4 claim(s) the database cannot corroborate:
