...
the daemon stops publishing it. Chained digest matches the graded rule: 353450b995a52bcea9257a8520369d8841e7fa71c70295aae925dc7cf03b429f.
  directional-ensemble (1d): n=2,670 over 21 days, design effect 7.3x -> effective n 366
  prequential-majority (1d): n=2,341 over 19 days, design effect 23.2x -> effective n 101
...
Reliability (predicted P(up) vs realized up-frequency, independent symbol-days):
  ...
  1d: p in [0.4,0.5) mean 0.468 -> realized 0.575 (n=1542, 19 days)
  ...
  1w: p in [0.0,0.1) mean 0.043 -> realized 0.333 (n=9, 4 days) <- conviction tier
  1w: p in [0.1,0.2) mean 0.176 -> realized 0.667 (n=81, 7 days) <- conviction tier
  1w: p in [0.2,0.3) mean 0.257 -> realized 0.562 (n=112, 12 days) <- conviction tier
  1w: p in [0.3,0.4) mean 0.364 -> realized 0.618 (n=1122, 18 days)
  1w: p in [0.4,0.5) mean 0.448 -> realized 0.585 (n=2093, 13 days)
  1w: p in [0.5,0.6) mean 0.545 -> realized 0.562 (n=1081, 13 days)
  1w: p in [0.6,0.7) mean 0.627 -> realized 0.636 (n=132, 8 days)
  1w: p in [0.7,0.8) mean 0.729 -> realized 0.381 (n=21, 6 days) <- conviction tier
  1w: p in [0.8,0.9) mean 0.818 -> realized 0.500 (n=2, 1 days) <- conviction tier
  1w: p in [0.9,1.0) mean 0.925 -> realized 0.000 (n=1, 1 days) <- conviction tier
...
Universe completeness (direction): NOT CLEAN — listing status resolvable for 328/651 graded symbols (50.4%): 323 inactive symbol(s) with no delisted_at. Rows carry survivorship_clean=false.
Universe completeness (structure): NOT CLEAN — unmeasured — no graded post-epoch symbols. Rows carry survivorship_clean=false.
Stale-feed quarantine: 398 graded observation(s) over 68 symbol(s) and 22 trading day(s) EXCLUDED - minted on a day this symbol's own feed was flagged stale (dq_events kind='stale'), so the inputs behind the call had already been declared unfit. Grading them would measure the outage, not the predictor.
Survivorship bound (measured 2026-08-23T21:05:05Z, edgar:form-25): 10 symbol(s) left the tracked universe since 2026-07-24 vs 651 graded. If every dropped symbol
had kept being graded and been WRONG every time, headline accuracy would fall by at most 0.70 pp.
