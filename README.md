...
> The grader refused at 2026-08-05T14:05:03 (research-loop liveness check failed (exit 1) — a narrated grid search left no verifiable judgment record, or a pre-registered forecast kind has never frozen a forecast and gave no refusal; the grader was not run). The last successful grade was 2026-08-04T20:34:19 (17.5h old). The previously published tables have been REMOVED rather than reprinted, because a number graded by code that refused to run today is not a live number.

```
research-loop liveness: 4 claim(s) ACKNOWLEDGED UNVERIFIABLE (quarantined, still reported, excluded from refusal):
  worker_runs id=130100 [missing-run] narrated a 48-rule grid search but no research_loop_runs row with judged>0 exists for UTC day(s) 2026-07-26
    narrated: searched a 48-rule grid over 163216 observations — NOTHING survived Bonferroni correction, regime-survival and the counterfactual. That is a result, not a failure: it is what an honest search returns …
    reason: research_loop_runs holds only a backfilled placeholder (git_rev=backfill:worker_runs, judged=0); no judgment was ever written
  worker_runs id=139413 [missing-run] narrated a 48-rule grid search but no research_loop_runs row with judged>0 exists for UTC day(s) 2026-07-27
    narrated: searched a 48-rule grid over 166285 observations — NOTHING survived Bonferroni correction, regime-survival and the counterfactual. That is a result, not a failure: it is what an honest search returns …
    reason: research_loop_runs holds only a backfilled placeholder (git_rev=backfill:worker_runs, judged=0); no judgment was ever written
  worker_runs id=145573 [judgment-count] narrated a 48-rule grid search on 2026-07-28 but research_loop_judgments holds 0 row(s) for that day
    narrated: searched a 48-rule grid over 166285 observations (search #3, Bonferroni divisor 144 = grid × searches) — NOTHING survived Bonferroni correction, regime-survival and the counterfactual. That is a resul…
    reason: run row claims judged=48 but research_loop_judgments holds 0 rows for the day; the judgments were never persisted
  worker_runs id=146527 [judgment-count] narrated a 48-rule grid search on 2026-07-29 but research_loop_judgments holds 0 row(s) for that day
    narrated: searched a 48-rule grid over 166367 observations (search #4, Bonferroni divisor 192 = grid × searches) — NOTHING survived Bonferroni correction, regime-survival and the counterfactual. That is a resul…
    reason: run row claims judged=48 but research_loop_judgments holds 0 rows for the day; the judgments were never persisted
RESEARCH-LOOP LIVENESS FAILED — 1 claim(s) the database cannot corroborate:
  prereg_records seq=37 [prereg-never-forecast] registered a forecast-producing hypothesis 1.0 days ago but regime_outcomes holds 0 rows of kind gradability-correction and no refusal naming it is on record — the registration is not falsifiable
    registered: kind=gradability-correction registered 2026-08-04 20:11 UTC
A search whose judgments cannot be counted from the database is an unverifiable null result, and a pre-registration that never forecast is an unfalsifiable one. Publication is refused; nothing is repaired here.
```
